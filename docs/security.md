...
| **Imported configuration** | Never executed. Parsed into a typed model through allow-listed fields, then regenerated. The XHTTP `extra` is sanitized, so nested `downloadSettings` cannot inject `masterKeyLog` (file write), `certificateFile` (file read) or `sockopt`. Unit tests assert that generated configs contain no `sockopt`, `masterKeyLog`, `certificateFile`, `allowInsecure`, API or 0.0.0.0. | OK |
| **Xray process arguments** | Fixed: `run -c stdin: -format json` (plus `-test` for validation). Credentials never appear on the command line. `XRAY_LOCATION_*` environment variables are removed. `CREATE_NO_WINDOW` on Windows. | OK |
...
| **Extension permissions** | `proxy`, `storage`, `nativeMessaging`, `activeTab`. `privacy` is optional and requested only when the user enables WebRTC protection. There are no host permissions. CSP for extension pages is `script-src 'self'; object-src 'none'; connect-src 'self' data:`. | OK |
...
| **Installer** | Per-user and writes only product-owned locations: its install dir, HKCU native messaging keys and HKCU Uninstall entry (Windows), and NativeMessagingHosts manifests (macOS). The Windows Mark-of-the-Web is stripped from installed binaries. The installer cannot be triggered by the browser (Chromium passes only the origin argument). | OK |
| **Dependency vulnerabilities** | `cargo audit` (RustSec, 1,251 advisories, 168 crates): **0 findings**. `npm audit`: **0 vulnerabilities**. Xray is pinned by version and SHA-256. TLS comes from the OS (SChannel / Security.framework); OpenSSL is not linked. | OK (2026-09-21) |
...
* **WebRTC.** Without the optional protection, sites can learn the real IP through WebRTC UDP.
